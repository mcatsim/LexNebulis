...
class TimeEntry(UUIDBase, TimestampMixin):
    ...
    invoice_id: Mapped[Optional[uuid.UUID]] = mapped_column(GUID(), ForeignKey("invoices.id"), nullable=True)
...
class Invoice(UUIDBase, TimestampMixin):
    ...
    invoice_number: Mapped[int] = mapped_column(Integer, unique=True, nullable=True)
    client_id: Mapped[uuid.UUID] = mapped_column(GUID(), ForeignKey("clients.id"), nullable=False, index=True)
    matter_id: Mapped[uuid.UUID] = mapped_column(GUID(), ForeignKey("matters.id"), nullable=False, index=True)
    issued_date: Mapped[Optional[_dt.date]] = mapped_column(Date, nullable=True)
    ...
    pdf_storage_key: Mapped[Optional[str]] = mapped_column(String(1000), nullable=True)
...
class InvoiceLineItem(UUIDBase):
    ...
    invoice_id: Mapped[uuid.UUID] = mapped_column(GUID(), ForeignKey("invoices.id", ondelete="CASCADE"), nullable=False)
    time_entry_id: Mapped[Optional[uuid.UUID]] = mapped_column(GUID(), ForeignKey("time_entries.id"), nullable=True)
    description: Mapped[str] = mapped_column(Text, nullable=False)
